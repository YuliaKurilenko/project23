...
def get_options_keyboard():
    ...
    flip_a_coin_btn = types.InlineKeyboardButton('Flip a coin', callback_data='flip')

    keyboard.add(pixelate_btn, ascii_btn, negative_btn, vertical_reflection_btn, horizontal_reflection_btn,
                 heat_map_btn, resize_for_stiker_btn, random_joke_btn, random_compliment_btn, flip_a_coin_btn)
    return keyboard
...
def callback_query(call):
    ...
    if call.data == "pixelate":
        ...
        bot.send_message(call.message.chat.id, choice(["Heads", "Tails"]))
